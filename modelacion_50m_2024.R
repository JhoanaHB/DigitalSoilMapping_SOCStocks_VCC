...
rfe <- rfe(Mat_Reg_3@data[,8:41], Mat_Reg_3@data[,7], sizes=c(1:34), rfeControl=control) #decir cuáles son mis variables predictoras (de columna 8 a 41, mi variable objetivo en la columna 7)
...
fm <- OCSKGMt ~ bio05+bio07+bio13+bio16+bio18 #son las que me salen con mis repeticiones
...
kr.cv=autoKrige.cv(OCSKGM ~ bio05+bio07+bio13+bio16+bio18, Mat_Reg_3)
class(kr.cv)
names(kr.cv)
